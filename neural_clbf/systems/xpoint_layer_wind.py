...
class XLayPoint(ControlAffineSystem):
    """
    ...
    def state_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[XLayPoint.X] = 7
        upper_limit[XLayPoint.Y] = 7
    ...
    def control_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[XLayPoint.UX] = 3 #100 #30 #15
        lower_limit = -1.0 * upper_limit
    ...
    def safe_mask(self, x):
        ...
        # within_x_bounds = (x[:, XLayPoint.X] >= x_lower_bound) & (x[:, XLayPoint.X] <= x_upper_bound)
        # within_y_bounds = (x[:, XLayPoint.Y] >= y_lower_bound) & (x[:, XLayPoint.Y] <= y_upper_bound)
    ...
    def _f(self, x: torch.Tensor, params: Scenario):
        ...
        # Use a sinusoidal function to determine the direction along the y-axis
        frequency_factor = 1.5
        f[:, XLayPoint.Y, 0] = 0.5 * torch.sin(frequency_factor * x[:, XLayPoint.X])
        
        # Normalize the result to ensure it points strongly up or down with magnitude 0.5
        f[:, XLayPoint.Y, 0] = 0.5 * torch.sign(f[:, XLayPoint.Y, 0])
        

        # Check if y is close to the goal[1] within a range of 0.5
        close_to_goal_y = torch.abs(x[:, XLayPoint.Y] - self.goal_point[0, XLayPoint.Y]) <= 0.5
        f[close_to_goal_y, XLayPoint.Y, 0] = 0.2 * torch.sign(self.goal_point[0, XLayPoint.X] - x[close_to_goal_y, XLayPoint.X])
        
        # Check if the state is close to the goal
        close_to_goal = (x - self.goal_point.type_as(x)).norm(dim=-1) <= 0.3
        f[close_to_goal, XLayPoint.Y, 0] = 0.0
    ...
    def _g(self, x: torch.Tensor, params: Scenario):
        ...
        g[:, XLayPoint.X, XLayPoint.UX] = 1.0
        
        return g
    
    # def u_nominal(self, x, params=None):
    #     hor = x[:, XLayPoint.X].type_as(x)
    ...
    def u_nominal(self, x, params=None):
        ...
        horizontal_diff = self.goal_point[0, XLayPoint.X].type_as(x) - x[:, XLayPoint.X]
